...
const QRCodePage = () => {
  ...
  const getShopkeeper = async () => {
    try{
      const url = `${apiUrl}/api/shopkeeper-profile`;
...
if (data) {
  try {
    const decodedBase64 = atob(decodeURIComponent(data)); // decode first
    const parsed = JSON.parse(decodedBase64); // parse JSON
    console.log('Decoded QR data:', parsed);
    setShopkeeperMobile(parsed.shopkeeper_mobile);
